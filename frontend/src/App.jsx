...
function App() {
  ...
  return <main className={`app ${theme}`}>
    ...
    <header className="hero">
      <div className="topbar"><div className="brand"><span className="ball">●</span> <em>CRICKONOMICS</em></div><div className="theme-picker" aria-label="Colour theme">{[['aurora', 'Aurora'], ['violet', 'Violet'], ['ember', 'Ember']].map(([value, label]) => <button key={value} className={theme === value ? 'active' : ''} onClick={() => setTheme(value)}>{label}</button>)}</div><span className="status"><i /> AI MODEL ONLINE (62.5% ACCURACY)</span></div>
    ...
    <footer>CRICKONOMICS <span>·</span> Not betting advice <span>·</span> Made with love❤️</footer>
  </main>
}
